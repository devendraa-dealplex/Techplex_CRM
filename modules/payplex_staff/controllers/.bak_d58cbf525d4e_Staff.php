...
class Staff extends AdminController
{
    ...
    {
        $staffId = (int) $staffId ?: $this->actor();
    ...
    {
        $staffId = (int) $staffId;
        if ($staffId) { $this->requireFieldScope($staffId); }
    ...
    public function expenses_export()
    {
        ...
        $rows = $this->m->claimsExport(array(
        ...
        $this->m->claimEvent(0, 'export', null, null, $me,
            ($viewAll ? 'All claims' : 'Own claims') . ' — ' . count($rows) . ' row(s)',
            null, null, array('scope' => $viewAll ? 'all' : 'own', 'rows' => count($rows)));
        ...
        header('X-Content-Type-Options: nosniff');
        header('Cache-Control: private, no-store');
        echo Workforce_expense::toCsv($rows);
        exit;
    }
}

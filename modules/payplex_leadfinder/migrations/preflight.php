 ...
 * WHAT THIS IS FOR
 * ----------------
 * Decision 1 requires a read-only duplicate preflight and a collision report
 * BEFORE migration 103 is applied. This is that runner, and it has since been
 * pointed at the later migrations that carry their own preflight — 112, 113 and
 * 114 — with `--migration=NNN`. It defaults to 103.
 *
 * Whichever migration is selected, it executes only that file's `preflight` and
 * `reports` queries. It cannot execute anything else, because it never reads
 * the statements a migration would apply.
 *
 ...
 * deciding not to apply 103 at all.
 *
 * SAFETY
 * ------
 *   - Refuses to run anything that is not a SELECT, checked per statement.
 ...
 *
 * USAGE
 *   php migrations/preflight.php --dsn="mysql:host=localhost;dbname=DB" \
 *       --user=USER --prefix=tbl --out=preflight-report.txt
 *
 * The password is read from the PREFLIGHT_DB_PASSWORD environment variable, not
 * from an argument: arguments appear in `ps` output and in shell history.
 */
 ...
 * did.
 */
$want = isset($opt['migration']) && $opt['migration'] !== '' ? $opt['migration'] : '103';
...
$out = array();
$out[] = "Lead Finder — migration preflight";
...
$out[] = '';
$out[] = 'This run wrote nothing. No lead was deleted, merged or overwritten.';

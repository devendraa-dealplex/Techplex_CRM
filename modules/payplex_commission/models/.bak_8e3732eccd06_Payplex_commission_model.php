    ...
    public function createPayoutBatch($period, $actorId = 0)
    {
        ...
        if (!$statements) {
            return array('ok' => false, 'errors' => array(
                'No payable statements for ' . $period . '. A statement must be approved and marked '
                . 'payable before it can be paid.'));
        }
        ...
        foreach ($statements as $s) {
            ...
            $item = array_merge($b, array(
                ...
                'staff_id'      => (int) $s['staff_id'],
                'gross_payable' => (float) $s['net_amount'],
                'other_deductions' => 0,
            ...
            $this->db->insert($this->tPItems(), $item);
            $count++;

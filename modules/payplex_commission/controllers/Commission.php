...
class Commission extends AdminController
{
    ...
    public function export_payout($id = 0)
    {
        ...
        /*
         * Both figures are recorded, not just the instructed one. A download of
         * this batch after a reversal previously logged net 3700.00 when only
         * 1850.00 was still out there, and the audit trail is the last place
         * that difference should be invisible.
         */
        $this->cmodel->payoutEvent((int) $id, null, 'export_downloaded', null, null, get_staff_user_id(),
            $export['has_reversed']
                ? 'Masked CSV export downloaded. Contains ' . (int) $export['reversed_count']
                  . ' reversed payment(s); instructed and standing totals differ.'
                : 'Masked CSV export downloaded.',
            array('rows' => $export['count'], 'net' => $export['total_net'],
                  'net_standing' => $export['total_standing'],
                  'net_reversed' => $export['total_reversed'],
                  'reversed_count' => $export['reversed_count']));
    ...
    {
        /*
         * Was gated on `pay`. Retrying a failed payment and recording that money
         * actually moved are different acts, and `payout_retry` now says so —
         * which also means the person who prepared the batch is not
         * automatically the person who can keep re-firing it at the bank.
         */
        if (!$this->cap('payout_retry')) { access_denied('payplex_commission'); }
     ...
     *
     * Its own capability, its own action, and a reason is mandatory. The gate
     * lives in the library and refuses the beneficiary regardless of what this
     * screen offers.
     */
    public function reverse_payout_item()
    {
        $itemId = (int) $this->input->post('item_id');
        $res = $this->cmodel->reversePayoutItem($itemId, get_staff_user_id(),
            $this->cap('payout_reverse'), (string) $this->input->post('reason'));

        set_alert(!empty($res['ok']) ? 'success' : 'danger', !empty($res['ok'])
            ? 'Payment reversed. The bank reference and amount are kept on the record.'
            : $res['reason']);
        redirect(admin_url('payplex_commission/commission/payout_detail/' . (int) $this->input->post('batch_id')));
    }
}

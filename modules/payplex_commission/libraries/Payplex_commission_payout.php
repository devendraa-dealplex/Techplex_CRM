...
class Payplex_commission_payout
{
    ...
    public static function exportColumns()
    {
        return array(
             ...
             *
             * Without it a reversed payment sits in the file looking exactly
             * like a live one. Measured on staging: batch PC-202609-00001 was
             * exported after item 1 was reversed, and the two rows were
             * indistinguishable — same beneficiary, same amount, nothing to say
             * that one of them had already come back.
             */
    ...
    public static function exportRow($batch, $item)
    {
        ...
        return array(
            ...
            'status'             => self::itemStatusLabel(isset($it['status']) ? $it['status'] : ''),
    ...
    /**
     * Instructed, reversed and still-standing money for a set of payout items.
     *
     * One definition, used by the export and by the batch screen, so the file
     * and the page can never disagree about what a batch actually moved.
     *
     * `instructed` deliberately includes reversed items: it is what the batch
     * told the bank to do, it is what the exported file said, and it must not
     * change retrospectively. `standing` is what is still out there. Reporting
     * only the first is how a reversal goes unnoticed at period end; reporting
     * only the second is how the instruction stops matching the file that was
     * actually sent.
     */
    public static function itemTotals($items)
    {
        $instructed = 0.0; $reversed = 0.0; $n = 0;
        foreach ((array) $items as $it) {
            $it   = (array) $it;
            $calc = self::computeNet($it);
            $net  = (float) $calc['net'];
        ...
        return array(
            'instructed'     => round($instructed, 2),
            'reversed'       => round($reversed, 2),
            'standing'       => round($instructed - $reversed, 2),
            'reversed_count' => $n,
        );
    }
    ...
    public static function buildExport($batch, $items)
    {
        ...
        /*
         * Three totals, because one is not enough once a reversal exists.
         *
         * `total_net` is what the batch INSTRUCTED — it must not change when a
         * payment is later reversed, or the file stops agreeing with the
         * instruction that was actually sent, and the audit event that recorded
         * the download would start meaning something different from the day it
         * was written.
         *
         * `total_standing` is what is still out there. On staging this batch
         * instructed 3,700.00 and 1,850.00 of it came back, so a single figure
         * of 3,700.00 on a reconciliation sheet is money that is not there.
         *
         * The reversed rows stay in the file. A reversal is a new fact about a
         * payment that happened, not an erasure of it, and dropping the line
         * would destroy the only record that the transfer was ever instructed.
         */
        $t = self::itemTotals($items);

        return array('ok' => true, 'columns' => $cols, 'rows' => $rows,
            'count'          => count($rows),
            'total_net'      => $t['instructed'],
            'total_reversed' => $t['reversed'],
            'total_standing' => $t['standing'],
            'reversed_count' => $t['reversed_count'],
            'has_reversed'   => $t['reversed_count'] > 0,
            'reason'         => '');
    }
    ...
    /**
     * Readable labels for the payment outcome.
     *
     * `reversed` is not labelled with the single word: the money went out and
     * came back, and somebody scanning a column of one-word statuses should not
     * have to remember that "reversed" implies "paid first". Both halves of the
     * fact are in the label because both halves are true.
     */
    public static function itemStatusLabel($status)
    {
        $map = array(
            self::ITEM_PENDING  => 'Pending',
            self::ITEM_PAID     => 'Paid',
            self::ITEM_FAILED   => 'Failed',
            self::ITEM_HELD     => 'Held',
            self::ITEM_REVERSED => 'Paid then reversed',
        );
        $s = (string) $status;

...
class Attendance_dashboard_model extends App_Model
{
    ...
    /** Company-wide default hours; used for new employees and by "apply to all". */
    public function defaultSchedule()
    {
        $days = get_option('att_default_days');

        return [
            'start' => get_option('att_default_start') ?: '09:00',
            'end'   => get_option('att_default_end') ?: '18:00',
            'days'  => array_map('intval', array_values(array_filter(explode(',', $days === '' ? '1,2,3,4,5' : $days), 'strlen'))),
        ];
    }

    /**
     * Saves the official hours. With $applyAll, overwrites the schedule of EVERY employee
     * (creating a schedule row for any employee that lacks one).
     * @return int|string  number of employees updated (0 when not applied), or an error message
     */
    public function saveDefaultSchedule($start, $end, array $days, $applyAll, $adminId)
    {
        if (!preg_match('/^\d{2}:\d{2}$/', $start) || !preg_match('/^\d{2}:\d{2}$/', $end) || $start === $end) {
            return 'Enter a valid, different start and end time.';
        }
        $days = array_values(array_intersect(range(1, 7), array_map('intval', $days)));
        if (!$days) {
            return 'Select at least one working day.';
        }
        $csv = implode(',', $days);
        update_option('att_default_start', $start);
        update_option('att_default_end', $end);
        update_option('att_default_days', $csv);
        if (!$applyAll) {
            return 0;
        }
        ...
        $this->db->trans_start();
        $this->db->where('1=1', null, false)->update($this->p . 'schedules', [
            'start_time' => $start . ':00', 'end_time' => $end . ':00', 'working_days' => $csv, 'updated_at' => $now,
        ]);
        $this->db->query("INSERT INTO {$this->p}schedules (employee_id, start_time, end_time, working_days, created_at, updated_at)
            SELECT e.id, ?, ?, ?, ?, ? FROM {$this->p}employees e LEFT JOIN {$this->p}schedules s ON s.employee_id = e.id WHERE s.id IS NULL",
            [$start . ':00', $end . ':00', $csv, $now, $now]);
        $this->db->trans_complete();
        if (!$this->db->trans_status()) {
            return 'Could not apply the hours to all employees.';
        }
        $n = $this->db->count_all($this->p . 'employees');
        log_activity('Attendance working hours applied to all ' . $n . ' employees: ' . $start . '-' . $end . ' days ' . $csv, $adminId);
    ...
    /**
     * Idempotent. Creates an employee (no workplace yet, official default working hours)
    ...
    public function syncStaff($staffId = 0)
    {
        ...
        foreach ($staff as $s) {
            ...
            $this->db->insert($this->p . 'schedules', [
                'employee_id' => $this->db->insert_id(), 'start_time' => $def['start'] . ':00', 'end_time' => $def['end'] . ':00',
                'working_days' => implode(',', $def['days']), 'created_at' => $now, 'updated_at' => $now,
            ]);
            $this->db->trans_complete();
        }
    }
    ...
    public function dayRows(array $f, $limit = 1000)
    {
        ...
        $leave = $this->leaveMap($f['from'], $f['to']); // approved leave can extend past "today"
        $in = $out = [];
        foreach ($recs as $r) {
        ...
        foreach ($emps as $e) {
            ...
            for ($t = strtotime($f['from']); $t <= strtotime($to); $t += 86400) {
                ...
                $rows[] = [
                    ...
                    'late' => $m['late'], 'early' => $m['early'], 'ot' => $m['ot'], 'leave_type' => $onLeave,
        ...
        if (in_array($want, ['present', 'late', 'absent', 'incomplete', 'working', 'on leave'], true)) {
    ...
    {
        return ['sick' => 'Sick Leave', 'casual' => 'Casual Leave', 'emergency' => 'Emergency Leave', 'unpaid' => 'Unpaid Leave', 'other' => 'Other'];
    }

    /** @return true|string  error message on failure */
    public function applyLeave($employeeId, $from, $to, $type, $reason, $appliedBy, $halfDay = false, $session = 'first')
    {
        $emp = $this->employee($employeeId);
        if (!$emp || !$emp['active']) {
        ...
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $from) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $to) || $to < $from) {
            return 'Enter a valid date range (to date on or after from date).';
        }
        if ($from < date('Y-m-d', strtotime('-1 year'))) {
            return 'The start date is too far in the past.';
        }
        $session = $session === 'second' ? 'second' : 'first';
        if ($halfDay) {
            if ($to !== $from) {
                return 'A half-day request must have the same from and to date.';
            }
            $days = 0.5;
        } else {
            $days = (int) ((strtotime($to) - strtotime($from)) / 86400) + 1;
            if ($days > 60) {
                return 'A single leave request cannot exceed 60 days.';
            }
        }
        if (!isset($this->leaveTypes()[$type])) {
            return 'Select a valid leave type.';
        }
        $reason = trim((string) $reason);
        if (mb_strlen($reason) > 500) {
            return 'Reason is too long.';
        }
        $overlap = $this->db->where('employee_id', (int) $employeeId)->where_in('status', ['pending', 'approved'])
            ->where('from_date <=', $to)->where('to_date >=', $from)->count_all_results($this->p . 'leaves');
        if ($overlap) {
            return 'An existing leave request already covers part of this date range.';
        }
        $now = date('Y-m-d H:i:s');
        $this->db->insert($this->p . 'leaves', [
            'employee_id' => (int) $employeeId, 'from_date' => $from, 'to_date' => $to,
            'is_half_day' => $halfDay ? 1 : 0, 'half_session' => $halfDay ? $session : null, 'days' => $days,
            'leave_type' => $type, 'reason' => $reason, 'status' => 'pending', 'applied_by' => (int) $appliedBy,
            'created_at' => $now, 'updated_at' => $now,
        ]);
        log_activity('Leave requested: ' . $emp['emp_code'] . ' ' . $from . ' to ' . $to . ($halfDay ? ' (half day)' : '') . ' (' . $type . ')');

        return true;
    }

    public function leaves(array $f = [], $limit = 500)
    {
        $this->db->select('l.*, e.full_name, e.emp_code')->from($this->p . 'leaves l')->join($this->p . 'employees e', 'e.id = l.employee_id', 'left');
        if (!empty($f['employee_id'])) {
            $this->db->where('l.employee_id', (int) $f['employee_id']);
        }
        if (!empty($f['status'])) {
            $this->db->where('l.status', $f['status']);
        }
    ...
    {
        return $this->db->select('l.*, e.full_name, e.emp_code')->from($this->p . 'leaves l')
            ->join($this->p . 'employees e', 'e.id = l.employee_id', 'left')->where('l.id', (int) $id)->get()->row_array();
    }

    /** @return true|string  error message on failure */
    public function reviewLeave($id, $status, $note, $adminId)
    {
        ...
        $this->db->where('id', (int) $id)->update($this->p . 'leaves', [
            'status' => $status, 'reviewed_by' => (int) $adminId, 'reviewed_at' => date('Y-m-d H:i:s'),
            'review_note' => mb_substr(trim((string) $note), 0, 255), 'updated_at' => date('Y-m-d H:i:s'),
        ]);
        log_activity('Leave ' . $status . ': ' . $row['emp_code'] . ' ' . $row['from_date'] . ' to ' . $row['to_date']);

        return true;
    }

    /** Employee cancels their own still-pending request. */
    public function cancelLeave($id, $employeeId)
    {
        $row = $this->leave($id);
        if (!$row || (int) $row['employee_id'] !== (int) $employeeId) {
            return 'Leave request not found.';
        }
        if ($row['status'] !== 'pending') {
            return 'Only a pending request can be cancelled.';
        }
        $this->db->where('id', (int) $id)->update($this->p . 'leaves', ['status' => 'cancelled', 'updated_at' => date('Y-m-d H:i:s')]);

        return true;
    }

    /** Approved-leave days overlapping the range, keyed employee_id|date => leave_type. */
    private function leaveMap($from, $to)
    {
        $rows = $this->db->where('status', 'approved')->where('from_date <=', $to)->where('to_date >=', $from)->get($this->p . 'leaves')->result_array();
        $map = [];
        foreach ($rows as $r) {
            $s = max(strtotime($from), strtotime($r['from_date']));
            $e = min(strtotime($to), strtotime($r['to_date']));
            for ($t = $s; $t <= $e; $t += 86400) {
                $map[$r['employee_id'] . '|' . date('Y-m-d', $t)] = $r['leave_type'];
            }
        }
    ...
    public function todayStats()
    {
        ...
        $leave = $this->leaveMap($today, $today);
        $scheduled = $present = $late = $notOut = $onLeave = 0;
        foreach ($emps as $e) {
            if (in_array(date('N'), explode(',', (string) $e['working_days']))) {
                ...
                } elseif (isset($leave[$k])) {
                    $onLeave++;
                }
            }
        }

        return ['total' => count($emps), 'present' => $present, 'absent' => $scheduled - $present - $onLeave, 'late' => $late, 'not_out' => $notOut, 'on_leave' => $onLeave];
    }
}

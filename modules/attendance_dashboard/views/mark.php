...
<div id="wrapper"><div class="content"><div class="row"><div class="col-md-6 col-md-offset-3">
    ...
    <div class="panel_s"><div class="panel-body">
        ...
        <div id="s1">
            ...
            <button class="btn btn-primary btn-block tw-mt-3" id="start">Continue &rarr; Open camera</button>
        ...
        <div id="s2" style="display:none">
            <p><strong id="who"></strong><br><small class="text-muted" id="meta"></small></p>
        ...
        <div id="s4" style="display:none;text-align:center">
            ...
            <button class="btn btn-primary" onclick="location.reload()">Mark another</button>
        </div>
    </div></div>
</div></div></div></div>
...
<script>
(function () {
    var base = '<?= admin_url(ATT_MODULE); ?>', token = null, empId = null, stream = null;
    ...
    $('#start').on('click', function () {
        ...
        if (!navigator.mediaDevices || !navigator.geolocation) { return err('This browser does not support camera and location. Use HTTPS and a modern browser.'); }
        $.post(base + '/begin', { employee_id: empId }, function (r) {
            if (!r.ok) { return err(r.reason); }
            token = r.token;
            $('#who').text(r.employee.name); $('#meta').text(r.employee.workplace + ' · ' + r.employee.window);
    ...
    $('#capture').on('click', function () {
        ...
        navigator.geolocation.getCurrentPosition(function (p) {
            btn.text('Verifying...');
            $.post(base + '/submit', { employee_id: empId, token: token, image: image, lat: p.coords.latitude, lng: p.coords.longitude, accuracy: p.coords.accuracy }, function (r) {
                stop();
                $('#resTitle').toggleClass('text-success', !!r.ok).toggleClass('text-danger', !r.ok).text(r.ok ? 'Attendance recorded' : 'Attendance not recorded');
                $('#resMsg').text(r.reason + (r.distance_m != null ? ' (' + r.distance_m + ' m from workplace)' : ''));
                show(4);
            }, 'json').fail(function () { fail('Request failed.'); });
        }, function () { btn.prop('disabled', false).text('Capture & verify location'); err('Location permission required. Allow location access and retry.'); },

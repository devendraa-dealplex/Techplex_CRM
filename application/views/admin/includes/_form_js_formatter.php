...
function addWidthSelector(fId)
{
    var field = $(fId);

    if(field.find('.width-selector').length) return;

    var html = `
    <div class="form-group width-selector">
        <label>Field Width</label>
        <select class="form-control width-select">
            <option value="col-12">100% Width</option>
            <option value="col-6">50% Width</option>
            <option value="col-4">33% Width</option>
            <option value="col-3">25% Width</option>
        </select>
    </div>
    `;

    field.find('.className-wrap').after(html);

    var select = field.find('.width-select');
    var classInput = field.find('input[name="className"]');

    select.on('change', function(){

        var width = $(this).val();

        classInput.val('form-control '+width);

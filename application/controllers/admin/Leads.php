...
class Leads extends AdminController
{
    ...
    public function convert_to_customer()
    {
        ...
        if ($this->input->post()) {
            ...
            // A converted lead gets ONE onboarding email: the branded "Welcome to
            // TechPlex" set-password email (contact-set-password), which also carries
            // the Video KYC link. The stock welcome email is suppressed so the customer
            // is not sent two. The form no longer posts a password or either checkbox;
            // force the flags here so a crafted request cannot change that.
            unset($data['password']);
            $data['donotsendwelcomeemail']   = 'on';

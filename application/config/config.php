...
|
*/
$config['log_path'] = '/home/y4l6swih97e7/crm_logs_staging/';
...
$config['csrf_exclude_uris'] = ['forms/wtl/[0-9a-z]+', 'forms/ticket', 'forms/quote/[0-9a-z]+', 'admin/tasks/timer_tracking', 'api\/.+', 'razorpay/success\/.+', 'leadgen_whatsapp/webhook', 'leadgen_facebook/webhook', 'leadgen_google/webhook.*'];
...
/*
 * payplex_contract_verification - inbound provider webhook.
 *
 * This ONE route is machine-to-machine: the signing provider posts to it with
 * no CRM session and no CSRF token, so CSRF would reject every delivery before
 * the module saw it. Excluded here, and nowhere else.
 *
 * It is a PUBLIC route (controllers/Webhook.php, extending App_Controller),
 * following the same convention as leadgen_facebook. The earlier admin path
 * could never work: Perfex's admin authentication redirects an unauthenticated
 * caller to the login page regardless of CSRF.
 *
 * CSRF protection itself is NOT disabled and no other URI is affected. What
 * CSRF incidentally provided is replaced, for this route only, by checks the
 * module enforces and which remain mandatory: signature verification that
 * fails closed, a 300s replay window checked both ways, idempotency on a
 * UNIQUE event-id key, a 1 MiB body limit checked before parsing, and a
 * recorded digest of every refused delivery.
 *
 * Added 2026-09-13. Reverse by deleting these lines. Backup of this file as it
 * stood immediately before: ~/pp_backups/config.php.20260913-pre-cv-csrf.bak
 */
$config['csrf_exclude_uris'][] = 'payplex_contract_verification/webhook';
